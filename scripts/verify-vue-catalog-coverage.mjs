...
if (actual !== expected) {
  throw new Error(`${artifact.path} is stale; run npm run generate:framework-artifacts`);
}

const indexSource = readFileSync(path.join(root, "packages/ui-vue/src/index.ts"), "utf8");
if (!indexSource.includes('export * from "./generated/catalog.generated";')) {
  throw new Error("Vue public entrypoint does not export the generated catalog");
}
const pluginSource = readFileSync(path.join(root, "packages/ui-vue/src/plugin.ts"), "utf8");
...
console.log(`Vue catalog coverage verified for ${artifact.components.length} supported non-grid contracts.`);
