...
const packages = [
  ...
  },
  {
    ...
    dependencies: ["@vyrnforge/ui-core", "@vyrnforge/ui-behaviors"],
    css: true,
    extraDistFiles: [
      "dist/register.js",
      "dist/register.cjs",
...
assert(
  rootPackageJson.license === licenseValue,
  `root package.json: license must be ${licenseValue}`,
);
assert(
  rootPackageJson.private === true,
  "root package.json: monorepo root must remain private",
);
...
for (const packageInfo of packages) {
  ...
  assert(
    ...
    `${packageInfo.name}: publishable package must not declare private`,
  );
  assert(
  ...
  assert(
    ...
    `${packageInfo.name}: license must be ${licenseValue}`,
  );
  assert(
    !conflictingSpdxLicenses.has(packageJson.license),
    `${packageInfo.name}: package uses conflicting SPDX license ${packageJson.license}`,
  );
  assert(
    packageJson.main === "./dist/index.cjs",
    `${packageInfo.name}: unexpected main`,
  );
  assert(
    packageJson.module === "./dist/index.js",
    `${packageInfo.name}: unexpected module`,
  );
  assert(
  ...
  if (packageInfo.css) {
    assert(
      Array.isArray(packageJson.sideEffects) &&
        packageJson.sideEffects.includes("./dist/index.css"),
      `${packageInfo.name}: CSS sideEffects must include ./dist/index.css`,
    );
  } else {
    assert(
      packageJson.sideEffects === false,
      `${packageInfo.name}: non-visual package must declare sideEffects false`,
    );
  }
  assert(
  ...
  assert(
    packageJson.repository?.directory === packageInfo.dir,
    `${packageInfo.name}: missing repository directory`,
  );
  assert(
    packageJson.bugs?.url ===
      "https://github.com/vyrnforge/vyrnforge-ui/issues",
    `${packageInfo.name}: missing bugs URL`,
  );
  assert(
    packageJson.publishConfig?.access === "public",
    `${packageInfo.name}: missing public publishConfig access`,
  );
  assert(
    packageJson.engines?.node === ">=22.12 <25",
    `${packageInfo.name}: node engine mismatch`,
  );
  assert(
    packageJson.engines?.npm === ">=11.16 <12",
    `${packageInfo.name}: npm engine mismatch`,
  );
  assert(
    existsSync(packageLicensePath),
    `${packageInfo.name}: missing top-level LICENSE`,
  );
  assert(
    readFileSync(packageLicensePath, "utf8") === rootLicenseText,
    `${packageInfo.name}: package LICENSE must exactly match root LICENSE`,
  );
  ...
  const requiredDistFiles = [
    ...baseRequiredDistFiles,
    ...(packageInfo.css ? ["dist/index.css"] : []),
    ...(packageInfo.extraDistFiles ?? []),
  ];
  for (const distFile of requiredDistFiles) {
    assert(
      existsSync(path.join(packageDir, distFile)),
      `${packageInfo.name}: missing ${distFile}`,
    );
  }

  assert(
    !existsSync(path.join(packageDir, "dist/index.js.map")),
    `${packageInfo.name}: source maps are not part of the supported package artifact`,
  );
  assert(
    !existsSync(path.join(packageDir, "dist/index.cjs.map")),
    `${packageInfo.name}: source maps are not part of the supported package artifact`,
  );
  ...
  assert(
    ...
    `${packageInfo.name}: root export types mismatch`,
  );
  assert(
    exportsMap["."]?.import === "./dist/index.js",
    `${packageInfo.name}: root export import mismatch`,
  );
  assert(
    exportsMap["."]?.require === "./dist/index.cjs",
    `${packageInfo.name}: root export require mismatch`,
  );
  if (packageInfo.css) {
    assert(
      exportsMap["./styles/index.css"] === "./dist/index.css",
      `${packageInfo.name}: styles export mismatch`,
    );
  } else {
    assert(
      !Object.hasOwn(exportsMap, "./styles/index.css"),
      `${packageInfo.name}: non-visual package must not export CSS`,
    );
  }
  ...
  for (const [exportKey, exportValue] of Object.entries(exportsMap)) {
    ...
    for (const target of targets) {
      assert(
        ...
        `${packageInfo.name}: unsupported export target for ${exportKey}`,
      );
      assert(
        !target.includes("/src/") && !target.startsWith("./src"),
        `${packageInfo.name}: public export points at src`,
      );
      assert(
        existsSync(path.join(packageDir, target)),
        `${packageInfo.name}: export target missing ${target}`,
      );
    }
  }
  ...
  for (const dependencyName of Object.keys(allDependencies)) {
    assert(
      ...
      `${packageInfo.name}: forbidden dependency ${dependencyName}`,
    );
  }
  ...
  for (const dependencyName of packageInfo.dependencies) {
    assert(
      packageJson.dependencies?.[dependencyName] ===
        packageByName.get(dependencyName).version,
      `${packageInfo.name}: internal dependency ${dependencyName} must match workspace version`,
    );
  }

  for (const peerName of packageInfo.peers ?? []) {
    assert(
      packageJson.peerDependencies?.[peerName] === ">=18 <20",
      `${packageInfo.name}: peer dependency ${peerName} must be >=18 <20`,
    );
    assert(
      packageJson.devDependencies?.[peerName],
      `${packageInfo.name}: missing dev copy for peer ${peerName}`,
    );
  }

  const distFiles = requiredDistFiles;
  for (const distFile of distFiles) {
    const text = readFileSync(path.join(packageDir, distFile), "utf8");
    assert(
      !/[A-Za-z]:\\\\|C:\\\\|D:\\\\|\/home\/runner|\/Users\//.test(text),
      `${packageInfo.name}: local absolute path leaked in ${distFile}`,
    );
  }
  ...
  for (const file of [
    ...
    assert(
      files.includes(file),
      `${packageInfo.name}: npm pack missing ${file}`,
    );
  }
  ...
  for (const file of files) {
    assert(
      ...
      `${packageInfo.name}: npm pack includes unexpected file ${file}`,
    );
    ...
    assert(
      file === "LICENSE" ||
        ...
        !/(legal|draft|internal|confidential)/i.test(file),
