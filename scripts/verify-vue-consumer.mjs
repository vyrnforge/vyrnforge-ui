...
export function verifyVueConsumer({ root = repositoryRoot } = {}) {
  ...
  for (const file of requiredFiles) {
    if (!existsSync(path.join(root, file)))
      fail(failures, `Vue consumer evidence is missing ${file}`);
  }
  ...
  if (framework.name !== "Vue" || framework.version !== vueVersion)
    fail(failures, `Vue framework version must be ${vueVersion}`);
  if (framework.supportLevel !== "first-class")
    fail(failures, "Vue support level must be first-class");
  if (framework.renderer !== "@vyrnforge/ui-vue")
    fail(failures, "Vue public renderer must be @vyrnforge/ui-vue");
  if (framework.workspaceIsolation !== true)
    fail(failures, "Vue fixture must remain workspace-isolated");
  if (metadata.fixture?.supportClaim !== expectedFixtureClaim)
    fail(failures, `Vue fixture claim must be ${expectedFixtureClaim}`);
  if (!metadata.fixture?.packedPackages?.includes("@vyrnforge/ui-vue"))
    fail(
      failures,
      "Vue fixture evidence must include the public facade package",
    );
  if (metadata.modelAdapterDecision?.publishedPackage !== "@vyrnforge/ui-vue")
    fail(failures, "Vue model integration must be package-owned");

  const fixture = readJson(root, "tests/consumers/vue/fixture.json");
  if (fixture.supportClaim !== expectedFixtureClaim)
    fail(failures, `Vue fixture support claim must be ${expectedFixtureClaim}`);
  if (!fixture.rendererPackages?.includes("@vyrnforge/ui-vue"))
    fail(failures, "Vue runtime fixture must exercise @vyrnforge/ui-vue");
  if (fixture.modelAdapterDecision !== "public-package-adapter-verified")
    fail(failures, "Vue fixture must use package-owned model integration");
  ...
  if (packageJson.dependencies?.["@vyrnforge/ui-elements"] === undefined)
    fail(failures, "@vyrnforge/ui-vue must delegate to @vyrnforge/ui-elements");
  if (packageJson.peerDependencies?.vue !== ">=3.5 <4")
    fail(failures, "@vyrnforge/ui-vue Vue peer range is invalid");
  ...
  for (const marker of ["createApp", "VyrnForgeVue", "@vyrnforge/ui-vue"]) {
    if (!mainText.includes(marker))
      fail(failures, `Vue entry point is missing ${marker}`);
  }
  if (mainText.includes("@vyrnforge/ui-elements/register"))
    fail(
      failures,
      "Vue normal setup must not require raw element registration",
    );
  ...
  for (const marker of [
    ...
    if (!appText.includes(marker))
      fail(failures, `Vue application is missing ${marker}`);
  }
  for (const stale of ["./adapters/", "useVyrnForgeModel"]) {
    if (appText.includes(stale))
      fail(failures, `Vue fixture must not depend on copied adapter ${stale}`);
  }
  ...
  if (
    vue?.supportLevel !== "first-class" ||
    vue?.renderer !== "@vyrnforge/ui-vue" ||
    vue?.betaClaim !== "first-class-package-verified"
  )
    fail(
      failures,
      "multi-framework Vue record must describe current first-class package support",
    );
