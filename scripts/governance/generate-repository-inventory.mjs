...
function walk(directory, match = () => true) {
  ...
  return readdirSync(absolute, { withFileTypes: true }).flatMap((entry) => {
    ...
    return entry.isDirectory() ? walk(file, match) : match(file) ? [file] : [];
...
function exportsFrom(file) {
  ...
  for (const match of source.matchAll(/export\s+(?:type\s+)?\{([\s\S]*?)\}\s+from\s+["'][^"']+["'];?/g)) {
    for (const member of match[1].split(",")) {
      const name = member.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*type\s+/, "").trim().split(/\s+as\s+/).at(-1)?.trim();
      if (/^[A-Za-z_$][\w$]*$/.test(name ?? "")) names.add(name);
    }
  }
  for (const match of source.matchAll(/export\s+(?:declare\s+)?(?:const|function|class|interface|type|enum)\s+([A-Za-z_$][\w$]*)/g)) names.add(match[1]);
...
function sourceFor(name, directory, files) {
  ...
  const matches = files.filter((file) => path.basename(file, path.extname(file)) === name);
  return matches.length === 1 ? matches[0] : "Requires verification";
}

function docsFor(name, directory, docs) {
  const candidates = [...docs, `packages/${directory}/README.md`].filter((file) => {
    return existsSync(path.join(root, file)) && (text(file).includes(`\`${name}\``) || text(file).includes(` ${name} `));
  });
...
function routeFor(name, routeFile) {
  ...
  const match = routeFile.match(new RegExp(`label:\\s*["']${escape}["']([\\s\\S]{0,500}?)Component:`, "m"));
  return match?.[1].match(/path:\s*["']([^"']+)["']/)?.[1] ?? "No dedicated route found";
}

function testFor(name, files) {
  const matches = files.filter((file) => text(file).includes(name));
  if (!matches.length) return ["No focused test found", "Requires verification"];
...
const components = records.filter((record) => record.directory !== "ui-core").flatMap((record) => {
  const files = walk(`packages/${record.directory}/src`, (file) => /\.(ts|tsx)$/.test(file));
  ...
  return [...names].sort((a, b) => a.localeCompare(b)).map((name) => {
    ...
    const gaps = [
      source === "Requires verification" && "Source mapping requires verification",
      documentation === "Requires verification" && "No component-specific documentation evidence",
      playground === "No dedicated route found" && "No dedicated playground route",
...
const outputLines = [
  ...
  table(["Area", "Measured inventory"], [
    ...
    ["Packages", records.map((record) => `\`${record.manifest.name}\``).join(", ")],
    ["Documentation", `${docs.length} active Markdown files; ${allDocs.length - docs.length} archived Markdown files`],
    ["Scripts", `${Object.keys(rootPackage.scripts ?? {}).length} root npm scripts; repository automation under \`scripts/\``],
    ["GitHub workflows", `${workflows.length} workflow files, including ${workflows.filter((file) => path.basename(file).startsWith("_")).length} reusable workflows`],
  ...
  ...records.flatMap((record) => {
    const metadata = packageMetadata.find((item) => item.name === record.manifest.name);
    const sourceDirectories = ["adapters", "components", "core", "hooks", "internal", "state", "styles", "theme", "types", "utils"].filter((name) => existsSync(path.join(root, "packages", record.directory, "src", name))).map((name) => `src/${name}`);
    const css = Object.keys(record.manifest.exports ?? {}).filter((entry) => entry.endsWith(".css")).sort();
    const maturity = Object.entries(record.maturity).map(([name, count]) => `${name}: ${count}`).join(", ") || "Requires verification";
    return [`### ${record.manifest.name}`, "", table(["Field", "Inventory"], [
      ["Purpose", metadata?.purpose ?? "Requires verification"],
      ["Main source directories", codeList(sourceDirectories)],
      ["Public exports", `${record.exports.length} root names. ${codeList(record.exports)}`],
      ["CSS entry points", codeList(css)],
      ["Dependencies", codeList(Object.entries(record.manifest.dependencies ?? {}).map(([name, version]) => `${name}@${version}`))],
      ["Peer dependencies", codeList(Object.entries(record.manifest.peerDependencies ?? {}).map(([name, version]) => `${name}@${version}`))],
      ["Build script", `\`${record.manifest.scripts?.build ?? "Requires verification"}\``],
      ["Test script", `\`${record.manifest.scripts?.test ?? "Requires verification"}\``],
  ...
  "",
  table(["Component", "Package", "Source file", "Public", "Docs", "Playground", "Tests", "Test type", "Metadata maturity", "Observed gaps", "Suggested owner"], components),
  "",
  "## D. Data-grid Inventory",
  "",
  table(["Area", "Evidence-led inventory"], [
    ["Public entry points", `${records.find((record) => record.directory === "ui-data-grid").exports.length} package-root names; CSS: ${codeList(Object.keys(records.find((record) => record.directory === "ui-data-grid").manifest.exports).filter((entry) => entry.endsWith(".css")))}`],
    ["Main component", "`packages/ui-data-grid/src/components/UniversalDataGrid.tsx` is " + countLines("packages/ui-data-grid/src/components/UniversalDataGrid.tsx") + " lines and concentrates rendering, state coordination, column behavior, grouping, selection, pagination, and feature composition."],
    ["Supporting components", codeList(walk("packages/ui-data-grid/src/components", (file) => file.endsWith(".tsx")).map((file) => file.replace("packages/ui-data-grid/src/components/", "")))],
    ...
    ["Existing tests", `${gridFiles.filter((file) => /\.test\.(ts|tsx)$/.test(file)).length} grid test files, concentrated in pure core/state/adapter/theme modules plus public-hook coverage.`],
    ["Large-file concentration", largeGridFiles.map(([file, count]) => `\`${file}\` (${count} lines)`).join("; ")],
  ...
  table(["Area", "Measured evidence"], [
    ...
    ["DOM interaction", `${domTests} detected test files with DOM interaction helpers.`],
  ...
  "",
  table(["Workflow", "Purpose", "Classification"], workflows.map((file) => [
    `\`${file}\``, workflowPurposes[path.basename(file)] ?? "Requires verification", path.basename(file).startsWith("_") ? "Reusable workflow" : "Top-level workflow"
  ])),
  "",
  table(["Area", "Inventory"], [
  ...
  "## I. Ownership Matrix",
  "",
  table(["Owner group", "Suggested accountability"], [
