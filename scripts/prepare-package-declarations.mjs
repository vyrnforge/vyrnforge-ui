...
assert(
  existsSync(distDir),
  `declaration output directory is missing: ${distDir}`,
);

const declarationFiles = walk(distDir).filter((file) => file.endsWith(".d.ts"));
assert(
  declarationFiles.length > 0,
  `no declaration files were emitted under ${distDir}`,
);
assert(
  existsSync(path.join(distDir, "index.d.ts")),
  "package declaration entry dist/index.d.ts is missing",
);
...
for (const file of declarationFiles) {
  ...
  assert(
    ...
    `test declaration leaked into package output: ${relativePath}`,
  );
  assert(
    !/\.(?:test|spec)\.d\.ts$/.test(relativePath),
    `test declaration leaked into package output: ${relativePath}`,
  );
...
const declarationMaps = walk(distDir).filter((file) =>
  file.endsWith(".d.ts.map"),
);
assert(
  declarationMaps.length === 0,
  "declaration maps are not part of the published package payload",
);
