...
const expectedPackages = new Map([
  ["@vyrnforge/ui-core", { status: "current", betaIncluded: true, dependsOn: [] }],
  [
    "@vyrnforge/ui-behaviors",
    { status: "current", betaIncluded: true, dependsOn: ["@vyrnforge/ui-core"] },
  ],
  [
...
function verifyPackageTopology(root, failures, architecture) {
  const packages = new Map(
    (architecture.packages ?? []).map((packageInfo) => [packageInfo.name, packageInfo]),
  );

  for (const [packageName, expected] of expectedPackages) {
    const actual = packages.get(packageName);
    if (!actual) {
      addFailure(failures, `multi-framework package topology is missing ${packageName}`);
      continue;
    }
    if (actual.status !== expected.status) {
      addFailure(failures, `${packageName} must have status ${expected.status}`);
    }
    if (actual.betaIncluded !== expected.betaIncluded) {
      addFailure(failures, `${packageName} betaIncluded must be ${String(expected.betaIncluded)}`);
    }
    if (!sameMembers(normalizeSet(actual.dependsOn), new Set(expected.dependsOn))) {
      addFailure(
        failures,
  ...
  const packageMetadataByName = new Map(
    (packageMetadata.packages ?? []).map((packageInfo) => [packageInfo.name, packageInfo]),
  );
  for (const [packageName, expected] of expectedPackages) {
    ...
    if (actual.status !== expected.status) {
      addFailure(failures, `packages.json ${packageName} status must be ${expected.status}`);
    }
    if (!sameMembers(normalizeSet(actual.dependsOn), new Set(expected.dependsOn))) {
      addFailure(failures, `packages.json ${packageName} dependsOn does not match the approved topology`);
    }
  }
  ...
  if (
    ...
    addFailure(failures, "packages.json dataGridAlpha release group is invalid");
  }
}

function verifyFrameworkSupport(failures, architecture) {
  const frameworks = new Map(
    (architecture.frameworks ?? []).map((framework) => [framework.id, framework]),
  );
  for (const [frameworkId, supportLevel] of expectedFrameworks) {
    ...
    if (framework.supportLevel !== supportLevel) {
      addFailure(failures, `${frameworkId} support level must be ${supportLevel}`);
    }
    const expectedClaim = expectedBetaClaims.get(frameworkId);
    if (framework.betaClaim !== expectedClaim) {
      addFailure(failures, `${frameworkId} beta claim must be ${expectedClaim}`);
    }
  }
  if (frameworks.size !== expectedFrameworks.size) {
    addFailure(failures, "framework support must contain exactly React, native HTML, Angular, and Vue");
  }

  if (architecture.styling?.defaultDomMode !== "light-dom") {
    addFailure(failures, "Light DOM must remain the default native element mode");
  }
  if (architecture.styling?.shadowDomPolicy !== "component-level-exception-only") {
    addFailure(failures, "Shadow DOM policy must require a component-level exception");
  }
  if (architecture.consumerFixturePolicy?.manifest !== "tests/consumers/manifest.json") {
    addFailure(failures, "consumer fixture manifest path is invalid");
  }
  if (architecture.consumerFixturePolicy?.evidence !== "docs/metadata/consumer-foundations.json") {
    addFailure(failures, "consumer fixture policy must reference consumer-foundations.json");
  }
  if (architecture.consumerFixturePolicy?.angularEvidence !== "docs/metadata/angular-consumer.json") {
    addFailure(failures, "consumer fixture policy must reference angular-consumer.json");
  }
  if (architecture.consumerFixturePolicy?.angularFormsEvidence !== "docs/metadata/angular-forms-adapter.json") {
    addFailure(failures, "consumer fixture policy must reference angular-forms-adapter.json");
  }
  if (architecture.consumerFixturePolicy?.vueEvidence !== "docs/metadata/vue-consumer.json") {
    addFailure(failures, "consumer fixture policy must reference vue-consumer.json");
  }
}

function verifyComponentContracts(failures, contracts) {
  if (contracts.$schema !== "./component-contract.schema.json") {
    addFailure(failures, "component contracts must reference component-contract.schema.json");
  }
  ...
  for (const event of contracts.eventVocabulary ?? []) {
    if (!/^vf-[a-z0-9]+(?:-[a-z0-9]+)*$/.test(event.name ?? "")) {
      addFailure(failures, `invalid canonical event name ${String(event.name)}`);
      continue;
    }
    if (eventNames.has(event.name)) addFailure(failures, `duplicate canonical event ${event.name}`);
    eventNames.add(event.name);
    if (event.bubbles !== true || event.composed !== true) {
      addFailure(failures, `${event.name} must bubble and cross composition boundaries`);
    }
    if (!Array.isArray(event.detail)) addFailure(failures, `${event.name} must define detail`);
  }
  if ([...expectedEvents].some((eventName) => !eventNames.has(eventName))) {
    addFailure(failures, "canonical event vocabulary is missing required baseline events");
  }

  const slotNames = new Set();
  for (const slot of contracts.slotVocabulary ?? []) {
    if (slotNames.has(slot.name)) addFailure(failures, `duplicate canonical slot ${String(slot.name)}`);
    slotNames.add(slot.name);
  }
  if ([...expectedSlots].some((slotName) => !slotNames.has(slotName))) {
    addFailure(failures, "canonical slot vocabulary is missing required baseline slots");
  }
  ...
  if (!sameMembers(normalizeSet(formAssociation.modes), new Set(["none", "value", "submitter"]))) {
  ...
  for (const contract of contracts.componentContracts ?? []) {
    ...
    if (
      ...
      addFailure(failures, `${contract.id} has an invalid React framework mapping`);
    }
    ...
    if (
      ...
      addFailure(failures, `${contract.id} has an invalid current native renderer`);
    }
    for (const event of contract.events ?? []) {
      if (!eventNames.has(event?.name)) {
        addFailure(failures, `${contract.id} references unknown event ${String(event?.name)}`);
      }
    }
    for (const slot of contract.slots ?? []) {
      if (!slotNames.has(slot?.name)) {
        addFailure(failures, `${contract.id} references unknown slot ${String(slot?.name)}`);
      }
    }
    if (!(formAssociation.modes ?? []).includes(contract.form?.association)) {
      addFailure(failures, `${contract.id} has invalid form association ${String(contract.form?.association)}`);
    }
  }
  if (!sameMembers(representativeIds, new Set(["button", "tabs", "autocomplete", "dialog"]))) {
    addFailure(failures, "representative contracts must cover button, tabs, autocomplete, and dialog");
  }
}

function verifyConsumerFixtures(root, failures, architecture) {
  const manifestPath = architecture.consumerFixturePolicy?.manifest;
  if (manifestPath !== "tests/consumers/manifest.json") return;
  ...
  for (const fixture of manifest.fixtures ?? []) {
    ...
    const contractPath = path.join(fixtureDirectory, fixture.contractFile ?? "fixture.json");
    ...
    if (contract.id !== fixture.id) {
      addFailure(failures, `${fixture.id} fixture id does not match its contract`);
    }
    const expectedFixtureClaim = expectedFixtureClaims.get(fixture.id);
    if (fixture.supportClaim !== expectedFixtureClaim) {
      addFailure(failures, `${fixture.id} manifest support claim must be ${expectedFixtureClaim}`);
    }
    if (contract.supportClaim !== expectedFixtureClaim) {
      addFailure(failures, `${fixture.id} fixture support claim must be ${expectedFixtureClaim}`);
    }
    const exampleText = (fixture.exampleFiles ?? [])
      .map((file) => path.join(fixtureDirectory, file))
      .map((file) => {
        if (!existsSync(file)) {
          addFailure(failures, `${fixture.id} fixture example ${path.basename(file)} is missing`);
          return "";
        }
    ...
    for (const pattern of contract.requiredPatterns ?? []) {
      if (!exampleText.includes(pattern)) {
        addFailure(failures, `${fixture.id} fixture examples are missing required pattern ${pattern}`);
      }
    }
  }
  if (!sameMembers(fixtureIds, new Set(expectedFrameworks.keys()))) {
    addFailure(failures, "consumer fixture manifest must contain React, native HTML, Angular, and Vue");
  }
}

export function verifyMultiFrameworkArchitecture({ root = repositoryRoot } = {}) {
  const failures = [];
  for (const document of requiredDocuments) {
    if (!existsSync(path.join(root, document))) {
      addFailure(failures, `required multi-framework document is missing: ${document}`);
    }
  }
  ...
  if (contracts.catalogCoverage?.scopedComponentCount !== publicNonGrid.length) {
  ...
  for (const component of publicNonGrid) {
    if (!component.frameworkParity) {
      addFailure(failures, `${component.id} is missing frameworkParity metadata`);
    }
  }
...
export function assertMultiFrameworkArchitecture(options) {
  ...
  if (failures.length > 0) {
    throw new Error(`Multi-framework architecture verification failed:\n- ${failures.join("\n- ")}`);
  }
}
...
if (
  ...
  console.log(
    "Multi-framework architecture passed: current package topology, contracts, events, slots, forms, and consumer fixtures are aligned.",
  );
}
