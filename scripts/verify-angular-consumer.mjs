...
export function verifyAngularConsumer({ root = repositoryRoot } = {}) {
  ...
  for (const file of requiredFiles) {
    if (!existsSync(path.join(root, file)))
      fail(failures, `Angular consumer evidence is missing ${file}`);
  }
  ...
  if (framework.supportLevel !== "first-class")
    fail(failures, "Angular support level must be first-class");
  if (framework.renderer !== "@vyrnforge/ui-angular")
    fail(failures, "Angular public renderer must be @vyrnforge/ui-angular");
  ...
  if (framework.workspaceIsolation !== true)
    fail(failures, "Angular fixture must remain workspace-isolated");
  if (metadata.fixture?.supportClaim !== expectedFixtureClaim)
    fail(failures, `Angular fixture claim must be ${expectedFixtureClaim}`);
  if (!metadata.fixture?.packedPackages?.includes("@vyrnforge/ui-angular"))
    fail(
      failures,
      "Angular fixture evidence must include the public facade package",
    );
  if (metadata.formsAdapter?.publicEntrypoint !== "@vyrnforge/ui-angular/forms")
    fail(
      failures,
      "Angular Forms evidence must use the package Forms entrypoint",
    );

  const support = readJson(root, "docs/metadata/angular-support-evidence.json");
  if (
    support.package?.name !== "@vyrnforge/ui-angular" ||
    support.package?.published !== false
  )
    fail(
      failures,
      "Angular support evidence must identify the unpublished public facade package",
    );
  ...
  if (!fixture.rendererPackages?.includes("@vyrnforge/ui-angular"))
    fail(
      failures,
      "Angular runtime fixture must exercise @vyrnforge/ui-angular",
    );
  ...
  if (packageJson.dependencies?.["@vyrnforge/ui-elements"] === undefined)
    fail(
      failures,
      "@vyrnforge/ui-angular must delegate to @vyrnforge/ui-elements",
    );
  if (packageJson.peerDependencies?.["@angular/core"] !== ">=22 <23")
    fail(failures, "@vyrnforge/ui-angular Angular peer range is invalid");
  if (
    packageJson.peerDependencies?.["@angular/forms"] !== ">=22 <23" ||
    packageJson.peerDependenciesMeta?.["@angular/forms"]?.optional !== true
  )
    fail(
      failures,
      "@angular/forms must remain an optional Angular facade peer",
    );
  if (!packageJson.exports?.["./forms"])
    fail(failures, "@vyrnforge/ui-angular must expose the Forms entrypoint");
  ...
  for (const marker of [
    ...
    if (!mainText.includes(marker))
      fail(failures, `Angular bootstrap is missing ${marker}`);
  }
  for (const stale of [
    "CUSTOM_ELEMENTS_SCHEMA",
    "@vyrnforge/ui-elements/register",
  ]) {
    if (mainText.includes(stale))
      fail(failures, `Angular normal bootstrap must not require ${stale}`);
  }
  ...
  for (const marker of [
    ...
    if (!componentText.includes(marker))
      fail(failures, `Angular component is missing ${marker}`);
  }
  if (componentText.includes("CUSTOM_ELEMENTS_SCHEMA"))
    fail(failures, "Angular first-class fixture must remain schema-free");
  ...
  for (const marker of [
    ...
    if (!templateText.includes(marker))
      fail(failures, `Angular template is missing ${marker}`);
  }
  ...
  const angular = (architecture.frameworks ?? []).find(
    (item) => item.id === "angular",
  );
  if (
    angular?.supportLevel !== "first-class" ||
    angular?.renderer !== "@vyrnforge/ui-angular" ||
    angular?.betaClaim !== "first-class-package-verified"
  )
    fail(
      failures,
      "multi-framework Angular record must describe current first-class package support",
    );
  if (architecture.angularConsumer?.publishedPackage !== null)
    fail(failures, "Angular metadata must not claim external publication");
