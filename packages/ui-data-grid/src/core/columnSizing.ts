...
export function isColumnResizable<RowData extends Record<string, unknown>>(
  column: DataGridColumnDef<RowData>,
) {
  return column.resizable !== false;
}

export function resolveColumnMinWidth<RowData extends Record<string, unknown>>(
  column: DataGridColumnDef<RowData>,
) {
  return column.minWidth ?? defaultColumnMinWidth;
}

export function resolveColumnMaxWidth<RowData extends Record<string, unknown>>(
  column: DataGridColumnDef<RowData>,
) {
...
export function resolveColumnWidth<RowData extends Record<string, unknown>>(
  column: DataGridColumnDef<RowData>,
  sizing: DataGridColumnSizingState = {},
) {
  const widthFromState = isColumnResizable(column)
    ? sizing[column.id]
    : undefined;
...
export function resolveColumnSizing<RowData extends Record<string, unknown>>(
...
export function setColumnSize<RowData extends Record<string, unknown>>(
  columns: DataGridColumnDef<RowData>[],
  sizing: DataGridColumnSizingState,
  columnId: string,
  width: number,
