...
function FormPageContent() {
  ...
  return (
    <section className="vf-docs-example-panel vf-docs-example-form-grid">
      <div>
        <Heading size="md">Request form</Heading>
        <Text tone="muted">
          A create/edit pattern using completed native form primitives.
        </Text>
      </div>
      <Field id="request-title" label="Request title" required>
        {(controlProps) => (
          <TextInput
            {...controlProps}
            defaultValue="Provision reporting workspace"
          />
        )}
      </Field>
      <Field
        id="request-owner"
        label="Owner"
        description="Search the workspace owner directory."
      >
        {(controlProps) => (
          <Autocomplete
            {...controlProps}
      ...
      <div className="vf-docs-example-grid vf-docs-example-grid--two">
        ...
        <Field
          label="Risk score"
          htmlFor="risk-score"
          warning="Scores above 70 require audit review."
        >
          <NumberInput
            id="risk-score"
            mode="integer"
      ...
      <Field
        label="Roles"
        description="Selected roles render as compact chips."
      >
        <MultiSelect
      ...
      <Field
        id="justification"
        label="Justification"
        error="A justification is required before submission."
        required
      >
        {(controlProps) => (
          <TextInput {...controlProps} placeholder="Add business reason" />
        )}
      </Field>
      <Checkbox
      ...
      {apiError && (
        <InlineMessage variant="danger" title="Could not create request">
          Resolve the highlighted fields and try again. This persistent message
          stays in context while the toast is transient.
        </InlineMessage>
      )}
      <div className="vf-docs-example-inline-actions vf-docs-example-form-footer">
        <Button
        ...
        <Button leftSlot={<Icon name="Edit" />} variant="subtle">
          Save draft
        </Button>
      </div>
    </section>
