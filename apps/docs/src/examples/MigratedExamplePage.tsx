...
const examples = {
  "theme-modes": ThemeModesPage, density: DensityPage, "css-overrides": CssOverridePage,
  "grid-basic": BasicGridPage, "grid-columns": ColumnsPage, "grid-filtering": FilteringPage,
  "grid-selection": SelectionPage, "grid-grouping": GroupingPage, "grid-resizing": ResizingPage,
  "grid-themes": ThemesGridPage, "grid-states": GridStatesPage, "grid-stress": StressGridPage,
  "pattern-resource-list": ResourceListPage, "pattern-detail": DetailPage, "pattern-settings": SettingsPage,
  "pattern-form": FormPage, "pattern-filter-form": FilterFormPage, "pattern-assignments": AssignmentPatternsPage,
  "pattern-feedback-states": EmptyErrorLoadingPage, "pattern-admin-shell": AdminShellPage,
...
export function MigratedExamplePage({ exampleId }: { exampleId: string }) {
  ...
  if (!Example) return <Text tone="muted">This documentation example is unavailable.</Text>;
  return (
    <div className="vf-docs-reference-layout">
      <div className="vf-docs-reference">
        <Card className="vf-docs-reference__section" id="interactive-example" padding="lg">
          <Heading level={3} size="md">Interactive example</Heading>
          <Text tone="muted">This example runs directly inside the VyrnForge documentation app and shares its theme, navigation, and runtime.</Text>
          <div className="vf-docs-example-stage"><Example /></div>
        </Card>
      </div>
      <aside className="vf-docs-reference-outline" aria-label="On this example page">
        <Text size="sm" tone="muted">On this page</Text>
        <nav><ul><li><a href="#interactive-example">Interactive example</a></li></ul></nav>
      </aside>
    </div>
  );
}
