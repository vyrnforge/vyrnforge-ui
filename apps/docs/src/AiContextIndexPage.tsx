    ...
    <Stack gap="lg">
      <Card padding="lg">
        <Stack gap="md">
          ...
          <Text>
            AI consumers should start with <code>ai-context/index.json</code>,
            select the smallest relevant pattern, category, or component slice,
            and only escalate to architecture documents for cross-cutting
            decisions.
          </Text>
          <pre className="vf-docs-reference-code">
            <code>{`npm run query:ai-context -- --component button --framework react\nnpm run query:ai-context -- --pattern settings`}</code>
          </pre>
        </Stack>
      </Card>
      ...
      <Card padding="lg">
        ...
        <div className="vf-docs-reference__grid">
          <Card padding="md">
            ...
            <Text>{index.categories.length} discovery slices</Text>
          </Card>
          <Card padding="md">
            <Heading level={4} size="sm">
              Patterns
            </Heading>
            <Text>{index.patterns.length} task-oriented slices</Text>
          </Card>
          <Card padding="md">
            <Heading level={4} size="sm">
              Components
            </Heading>
            <Text>{index.components.length} focused component slices</Text>
          </Card>
          <Card padding="md">
            <Heading level={4} size="sm">
              Frameworks
            </Heading>
            <Text>{index.frameworks.map((entry) => entry.id).join(", ")}</Text>
          </Card>
        </div>
      </Card>
