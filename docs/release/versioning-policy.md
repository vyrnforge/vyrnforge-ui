...
dependencies. `@vyrnforge/ui-vue` depends directly on `@vyrnforge/ui-elements`
and keeps Vue as a peer dependency. The synchronized version does not promote
...
- `@vyrnforge/ui-vue` may depend on `ui-elements` and keeps Vue as a peer dependency.
