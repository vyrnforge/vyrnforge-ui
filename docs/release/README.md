...
VyrnForge UI is currently pre-alpha and source-available under the VyrnForge Source License 1.0. The `0.1.0-alpha.0` package set is the current alpha candidate; it has not been published to npm. These documents define the intended release process and gates, but they do not claim public package releases, stable support, trusted publishing, or commercial licensing infrastructure.
