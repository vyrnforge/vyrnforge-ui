...
| `CodeText` | Inline code-like text. | `import { CodeText } from "@vyrnforge/ui-components";`<br>`<CodeText>--dv-primary</CodeText>` | `children` | Keep code readable in context. | Use for tokens, package names, and snippets. Avoid large blocks. |
...
| `SideNav` | Sidebar navigation with active, disabled, badge, icon, collapsed, header/footer slots, and one-level nested items. | `import { SideNav } from "@vyrnforge/ui-components";`<br>`<SideNav activeId="orders" items={items} onSelect={setRoute} />` | `items`, `activeId`, `collapsed`, `onSelect`, `header`, `footer` | Uses a nav landmark and `aria-current` for active items. | Use for app navigation. The item list scrolls inside `dv-side-nav__scroll` so header/footer remain visible. Avoid for complex tree views. |
