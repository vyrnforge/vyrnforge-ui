...
| `Switch` | On/off setting toggle. | `import { Switch } from "@dravyn/ui-components";`<br>`<Switch checked={enabled} onCheckedChange={setEnabled} label="Enabled" />` | `checked`, `defaultChecked`, `onCheckedChange`, `label`, `description` | Uses native checkbox with `role="switch"`. | Use for immediate settings. Avoid for acknowledgements; use `Checkbox`. |
...
| `ValidationMessage` | Inline validation/supporting message. | `import { ValidationMessage } from "@dravyn/ui-components";`<br>`<ValidationMessage tone="error">Required</ValidationMessage>` | `tone`, native div props | Error tone uses alert semantics and can be referenced by `aria-describedby`. | Use for custom field feedback. Prefer `Field error` for standard field errors. |
