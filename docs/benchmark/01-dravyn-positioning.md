...
Dravyn UI is not only a data grid package. The data grid is the first strategic specialized component, but the product direction is broader: shared tokens, reusable application components, and specialized enterprise data-management surfaces.

## What Dravyn Is

| Identity | Status | Meaning |
| --- | --- | --- |
| Data-grid library | Yes, but not only that | `@dravyn/ui-data-grid` provides a specialized enterprise grid. |
| Component library | Yes | `@dravyn/ui-components` provides reusable React primitives and application components. |
| Design system foundation | Yes | `@dravyn/ui-core` provides tokens, themes, density, and utilities. |
...
| CSS framework | No | Dravyn does not aim to replace Bootstrap or Tailwind as a general CSS utility layer. |
| Headless primitive library | No | Dravyn may expose controlled APIs, but it ships styled enterprise defaults. |
| App state framework | No | Dravyn does not own global state, fetching, permissions, auth, or business workflows. |

## Package Roles

| Package | Role | Owns | Does not own |
| --- | --- | --- | --- |
| `@dravyn/ui-core` | Shared foundation | Tokens, themes, density, utilities, shared CSS variables | React components, grid logic, app state |
| `@dravyn/ui-components` | Reusable UI layer | Native React primitives, application components, shared `dv-*` styles | Data-grid behavior, backend data, global store |
| `@dravyn/ui-data-grid` | Specialized data-management package | UniversalDataGrid, grid state contracts, grid-specific algorithms, adapters, `udg-*` styles | App data fetching, report generation, business workflows |
...
| Store-agnostic | No Redux/Zustand/TanStack state dependency inside Dravyn packages. |
...
| Compared with | Dravyn positioning |
| --- | --- |
...
| Radix / React Aria / Headless UI | Less headless; Dravyn ships usable visual defaults. Should learn accessibility depth without taking dependency by default. |
| shadcn/ui | Versioned package model rather than copy-paste ownership. Should learn transparent, readable implementation. |
| TanStack Table | Dravyn owns a styled grid, not only a headless engine. Should learn controlled state and row model clarity. |
...
Dravyn should not optimize for:
