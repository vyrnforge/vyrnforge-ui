...
const formImport = 'import { Field, TextInput } from "@dravyn/ui-components";';
...
export function FieldReferencePage() {
  ...
  return <ComponentDemoPage
    ...
    description="Label, description, required-state, and validation composition for native Dravyn controls."
...
</Stack>`, scope, 'import { Field, Stack, TextInput } from "@dravyn/ui-components";') }
    ]}
    status="experimental"
    title="Field"
...
</Inline>`, scope, 'import { Inline, ValidationMessage } from "@dravyn/ui-components";') }]} status="experimental" title="ValidationMessage" useWhen={["Explaining an individual validation state.", "Providing compact field-adjacent guidance."]} />;
}

export function RadioPage() {
  const scope = createLiveScope("Radio", "Stack");
  return <ComponentDemoPage accessibility={["Give an individual radio a visible label or aria-label.", "Group mutually exclusive options with RadioGroup."]} avoidWhen={["A binary setting; use Checkbox or Switch.", "A grouped choice; use RadioGroup."]} description="A native radio input for simple, explicitly named single-choice options." importCode={'import { Radio } from "@dravyn/ui-components";'} packageName="@dravyn/ui-components" props={[{ name: "checked | defaultChecked", type: "boolean", description: "Controlled or initial selected state." }, { name: "name", type: "string", description: "Native radio group name." }, { name: "label", type: "ReactNode", description: "Visible option label." }, { name: "description", type: "ReactNode", description: "Optional supporting text." }]} relatedComponents={[]} sections={[{ id: "basic-usage", label: "Basic usage", title: "Basic usage", children: live("radio-basic-live", "Standalone option", '<Radio defaultChecked label="Monthly billing" name="billing" value="monthly" />', scope, 'import { Radio } from "@dravyn/ui-components";') }, { id: "states", label: "States", title: "States", children: live("radio-states-live", "Disabled and invalid", `<Stack gap="sm">
...
</Stack>`, scope, 'import { Radio, Stack } from "@dravyn/ui-components";') }]} status="experimental" title="Radio" useWhen={["Composing a small native choice set manually.", "A RadioGroup is not needed because the surrounding fieldset is already present."]} />;
}

export function RadioGroupPage() {
  const scope = createLiveScope("RadioGroup", "Stack");
  return <ComponentDemoPage accessibility={["RadioGroup uses a fieldset and legend for its choice set.", "Native arrow-key behavior remains available within the group."]} avoidWhen={["A long option list; use Select.", "Independent multiple selections; use Checkbox."]} description="A fieldset-based native radio group with controlled or uncontrolled value support." importCode={'import { RadioGroup } from "@dravyn/ui-components";'} packageName="@dravyn/ui-components" props={[{ name: "options", type: "RadioGroupOption[]", required: true, description: "Value, label, description, and disabled state." }, { name: "value | defaultValue", type: "string", description: "Controlled or initial selection." }, { name: "onValueChange", type: "(value: string) => void", description: "Selection callback." }, { name: "orientation", type: '"vertical" | "horizontal"', defaultValue: '"vertical"', description: "Option layout." }]} relatedComponents={[]} sections={[{ id: "basic-usage", label: "Basic usage", title: "Basic usage", children: live("radio-group-basic-live", "Delivery choice", '<RadioGroup label="Delivery method" defaultValue="standard" name="delivery" options={[{ label: "Standard", value: "standard" }, { label: "Express", value: "express" }]} />', scope, 'import { RadioGroup } from "@dravyn/ui-components";') }, { id: "validation", label: "Validation", title: "Validation", children: live("radio-group-validation-live", "Required horizontal selection", `<Stack gap="md">
...
</Stack>`, scope, 'import { RadioGroup, Stack } from "@dravyn/ui-components";') }]} status="experimental" title="RadioGroup" useWhen={["One option must be selected from a small named set.", "The form needs a visible group label and validation text."]} />;
}

export function SwitchPage() {
  const scope = createLiveScope("Stack", "Switch");
  return <ComponentDemoPage accessibility={["Switch uses a native checkbox with role=switch and keyboard support.", "The on and off labels make state clear without relying only on color."]} avoidWhen={["Acknowledging a policy; use Checkbox.", "Selecting one option from a set; use RadioGroup."]} description="A native checkbox-based switch for immediate settings and preferences." importCode={'import { Switch } from "@dravyn/ui-components";'} packageName="@dravyn/ui-components" props={[{ name: "checked | defaultChecked", type: "boolean", description: "Controlled or initial enabled state." }, { name: "onCheckedChange", type: "(checked: boolean) => void", description: "Boolean state callback." }, { name: "label", type: "ReactNode", description: "Visible setting label." }, { name: "description", type: "ReactNode", description: "Supporting setting detail." }]} relatedComponents={[]} sections={[{ id: "basic-usage", label: "Basic usage", title: "Basic usage", children: live("switch-basic-live", "Preference", '<Switch defaultChecked label="Enable workflow alerts" description="Notify request owners when status changes." />', scope, 'import { Switch } from "@dravyn/ui-components";') }, { id: "states", label: "States", title: "States", children: live("switch-states-live", "Controlled and unavailable settings", `function Example() {
...
render(<Example />);`, scope, 'import { Stack, Switch } from "@dravyn/ui-components";') }]} status="experimental" title="Switch" useWhen={["Changing an immediate, independent setting.", "The setting is understandable in both on and off states."]} />;
}
...
function nativeInputPage(
  ...
  const imports = `import { ${component}, Field, Stack } from "@dravyn/ui-components";`;
  return <ComponentDemoPage accessibility={["Provide a visible Field label or aria-label.", "Native constraints such as required, min, max, and readOnly stay available."]} avoidWhen={["The value needs custom formatting or masking."]} description={description} importCode={imports} packageName="@dravyn/ui-components" props={props} relatedComponents={[]} sections={[{ id: "basic-usage", label: "Basic usage", title: "Basic usage", children: live(`${component.toLowerCase()}-basic-live`, "Native input", basicCode, scope, imports) }, { id: "states", label: "States", title: "States", children: live(`${component.toLowerCase()}-states-live`, "Required, invalid, and unavailable", stateCode, scope, imports) }]} status="experimental" title={title} useWhen={["A standard native input covers the workflow.", "The application benefits from browser-native editing and validation behavior."]} />;
}
...
export function TextInputReferencePage() {
  const scope = createLiveScope("Field", "Stack", "TextInput");
  return <ComponentDemoPage accessibility={["Provide Field label or aria-label.", "Pair invalid state with clear associated validation text."]} avoidWhen={["The value is a search query; use SearchInput.", "The value spans multiple lines; use Textarea."]} description="A native text input with shared Dravyn sizes, focus styling, and Field support." importCode={formImport} packageName="@dravyn/ui-components" props={[{ name: "size", type: '"sm" | "md" | "lg"', defaultValue: '"md"', description: "Control density." }, { name: "invalid", type: "boolean", description: "Invalid visual and ARIA state." }, { name: "readOnly | disabled | required", type: "boolean", description: "Native input states." }]} relatedComponents={[]} sections={[{ id: "basic-usage", label: "Basic usage", title: "Basic usage", children: live("text-input-field-live", "Label and description", `<Field id="workspace-name" label="Workspace name" description="Used in the workspace URL." required>
  {(controlProps) => <TextInput {...controlProps} defaultValue="Operations" />}
...
</Stack>`, scope, 'import { Stack, TextInput } from "@dravyn/ui-components";') }]} status="experimental" title="TextInput" useWhen={["Capturing a short text value.", "The application needs native onChange behavior."]} />;
}

export function TextareaPage() {
  const scope = createLiveScope("Field", "Stack", "Textarea");
  return <ComponentDemoPage accessibility={["Provide Field label or aria-label.", "Textarea keeps native keyboard and resize behavior."]} avoidWhen={["The value is a short single-line field; use TextInput."]} description="A native multiline text control with shared Dravyn styling and validation states." importCode={'import { Field, Textarea } from "@dravyn/ui-components";'} packageName="@dravyn/ui-components" props={[{ name: "rows", type: "number", description: "Native visible line count." }, { name: "size", type: '"sm" | "md" | "lg"', defaultValue: '"md"', description: "Control density." }, { name: "invalid", type: "boolean", description: "Invalid visual and ARIA state." }]} relatedComponents={[]} sections={[{ id: "basic-usage", label: "Basic usage", title: "Basic usage", children: live("textarea-basic-live", "Justification", `<Field id="justification" label="Justification" description="Explain the business reason for this request.">
  {(controlProps) => <Textarea {...controlProps} defaultValue="Access is needed for month-end reconciliation." rows={4} />}
</Field>`, scope, 'import { Field, Textarea } from "@dravyn/ui-components";') }, { id: "states", label: "States", title: "States", children: live("textarea-states-live", "Required and unavailable", `<Stack gap="sm">
...
</Stack>`, scope, 'import { Stack, Textarea } from "@dravyn/ui-components";') }]} status="experimental" title="Textarea" useWhen={["Capturing longer explanations, notes, or justifications."]} />;
}

export function SelectReferencePage() {
  const scope = createLiveScope("Field", "Select", "Stack");
  return <ComponentDemoPage accessibility={["Associate Select with Field or aria-label.", "Native select provides its own keyboard behavior."]} avoidWhen={["The list is large or dynamic.", "Multiple selections are required."]} description="A native select for short, stable option sets with shared size and validation styling." importCode={'import { Field, Select } from "@dravyn/ui-components";'} packageName="@dravyn/ui-components" props={[{ name: "options", type: "SelectOption[]", description: "Option label, value, and disabled state." }, { name: "size", type: '"sm" | "md" | "lg"', defaultValue: '"md"', description: "Control density." }, { name: "invalid", type: "boolean", description: "Invalid visual and ARIA state." }]} relatedComponents={[]} sections={[{ id: "basic-usage", label: "Basic usage", title: "Basic usage", children: live("select-field-live", "Default region", `<Field id="region" label="Default region" required>
...
</Field>`, scope, 'import { Field, Select } from "@dravyn/ui-components";') }, { id: "states", label: "States", title: "States", children: live("select-states-live", "Invalid and disabled options", `<Stack gap="sm">
...
</Stack>`, scope, 'import { Select, Stack } from "@dravyn/ui-components";') }]} status="experimental" title="Select" useWhen={["Choosing one option from a short stable list."]} />;
}

export function CheckboxReferencePage() {
  const scope = createLiveScope("Checkbox", "Field", "Stack");
  return <ComponentDemoPage accessibility={["A label is preferred unless surrounding text already names the control.", "Use invalid state with nearby validation guidance."]} avoidWhen={["Choosing one exclusive option; use RadioGroup.", "Toggling an immediate setting; use Switch."]} description="A native checkbox for acknowledgements and independent choices." importCode={'import { Checkbox } from "@dravyn/ui-components";'} packageName="@dravyn/ui-components" props={[{ name: "checked | defaultChecked", type: "boolean", description: "Controlled or initial state." }, { name: "label", type: "ReactNode", description: "Visible checkbox label." }, { name: "size", type: '"sm" | "md" | "lg"', defaultValue: '"md"', description: "Control density." }, { name: "invalid", type: "boolean", description: "Invalid visual and ARIA state." }]} relatedComponents={[]} sections={[{ id: "basic-usage", label: "Basic usage", title: "Basic usage", children: live("checkbox-basic-live", "Acknowledgement", '<Checkbox defaultChecked label="I agree to the workspace policy" />', scope, 'import { Checkbox } from "@dravyn/ui-components";') }, { id: "validation", label: "Validation", title: "Validation", children: live("checkbox-states-live", "Required, invalid, and disabled", `<Stack gap="sm">
...
</Stack>`, scope, 'import { Checkbox, Field, Stack } from "@dravyn/ui-components";') }]} status="experimental" title="Checkbox" useWhen={["Acknowledging a statement.", "Selecting several independent options."]} />;
}
