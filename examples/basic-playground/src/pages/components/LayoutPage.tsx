  ...
  return (
    ...
    >
      <DemoSection description="Cards and panels provide the surfaces; Stack and Inline provide rhythm without one-off CSS." title="Composition">
        <DemoBlock code={'<Panel title="Workspace settings">\n  <Stack gap="md">\n    <Inline justify="between">...</Inline>\n  </Stack>\n</Panel>'} preview={<Section title="Section" description="Section provides title, description, actions, and content layout." actions={<Button size="sm" variant="primary">Action</Button>}><Panel title="Panel" description="Panel frames a related task area."><Stack gap="md"><Inline justify="between"><Text tone="strong">Inline row</Text><Badge variant="info">metadata</Badge></Inline><div className="dv-playground-demo-grid"><Card variant="plain" padding="md"><Text>Plain card</Text></Card><Card variant="bordered" padding="md"><Text>Bordered card</Text></Card><Card variant="elevated" padding="md"><Text>Elevated card</Text></Card></div></Stack></Panel></Section>} title="Panels, stacks, and cards" />
      </DemoSection>
      <DemoSection description="The shell keeps the sidebar and content regions responsible for their own scrolling." title="Application shell">
        <DemoBlock code={'<AppShell header={<TopNav brand="Operations" />} sidebar={<SideNav items={items} />} scrollMode="content" sidebarPosition="sticky">\n  <Page>...</Page>\n</AppShell>'} preview={<AppShell className="dv-playground-shell-preview" fullHeight={false} header={<TopNav brand="Operations" actions={<Badge tone="subtle">sticky</Badge>} />} headerPosition="sticky" minHeight={420} scrollMode="content" sidebar={<SideNav activeId="queue-3" items={shellNavItems} />} sidebarPosition="sticky" sidebarWidth={190}><div className="dv-playground-shell-preview__content"><PageToolbar sticky left={<Text tone="strong">Toolbar</Text>} right={<Badge variant="info">content scroll</Badge>} />{longShellRows.map((row) => <Panel key={row} title={row}><Text tone="muted">Long content confirms the sidebar stays visible while the work area scrolls.</Text></Panel>)}</div></AppShell>} title="Persistent navigation" />
      </DemoSection>
    </DemoPage>
  );
}
