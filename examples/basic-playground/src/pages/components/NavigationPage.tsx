  ...
  return (
    ...
    >
      ...
      <DemoSection description="Use a persistent side navigation for major destinations and tabs for related content on the same page." title="Persistent and local navigation">
        <div className="dv-playground-demo-grid">
          <DemoBlock code={'<SideNav activeId={activeId} items={items} onSelect={handleSelect} />'} preview={<div className="dv-playground-nav-demo-frame"><SideNav activeId={activeNav} collapsed={collapsed} items={navItems} onSelect={(item) => setActiveNav(item.id)} /></div>} title="Side navigation" />
          <DemoBlock code={'<Tabs value={activeTab} onValueChange={setActiveTab} items={items} />'} preview={<div className="dv-playground-page-stack"><PageToolbar left={<SearchInput aria-label="Search records" placeholder="Search records" />} right={<><ToolbarButton icon={<Icon name="Refresh" />} label="Refresh" /><ToolbarButton active icon={<Icon name="Filter" />} label="Filters" /></>} /><Tabs value={activeTab} onValueChange={setActiveTab} items={[{ id: "activity", label: "Activity", badge: <Badge size="sm">8</Badge>, content: <Text>Activity tab content follows the controlled value.</Text> }, { id: "details", label: "Details", content: <Text>Details tab content is rendered as a tab panel.</Text> }, { id: "audit", label: "Audit", disabled: true }]} /><ToolbarButton active={collapsed} icon={<Icon name={collapsed ? "ChevronRight" : "ChevronLeft"} />} label={collapsed ? "Expand navigation" : "Collapse navigation"} onClick={() => setCollapsed((value) => !value)} size="sm" /></div>} title="Toolbar and tabs" />
        </div>
      </DemoSection>
    </DemoPage>
  );
