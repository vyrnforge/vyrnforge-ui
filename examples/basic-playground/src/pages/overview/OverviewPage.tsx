    ...
    <Stack gap="lg">
      <section className="vf-playground-panel vf-playground-overview-hero">
        <Stack gap="md">
          ...
          <Text tone="muted">
            Explore reusable components, application patterns, shared
            foundations, and optional advanced modules. Examples use public
            VyrnForge APIs and keep business state in the consuming application.
          </Text>
          <Inline gap="sm" wrap>
            <Button onClick={() => navigate("/settings")} variant="primary">
              Explore a settings workflow
            </Button>
            <Button
              onClick={() => navigate("/components/actions/button")}
              variant="subtle"
            >
              Browse components
            </Button>
      ...
      <section>
        ...
        <Text tone="muted">
          Start from a reusable application composition, then drill into the
          components it uses.
        </Text>
      ...
      <section>
        ...
        <Text tone="muted">
          Support labels come from canonical multi-framework metadata rather
          than playground copy.
        </Text>
      ...
      <section>
        ...
        <Text tone="muted">
          The playground is broader than the grid: shared foundations,
          renderers, and advanced modules have separate responsibilities and
          release tracks.
        </Text>
        <div className="vf-playground-grid vf-playground-grid--three">
          {knowledge.packages.map((packageEntry) => (
            <Card key={packageEntry.name} padding="md">
              <Stack gap="xs">
                ...
                {packageEntry.releaseTrack && (
                  <Badge tone="subtle">{packageEntry.releaseTrack}</Badge>
                )}
